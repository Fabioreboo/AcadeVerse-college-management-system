...
import { User, StudentRecord, Subject, MarkRecord, AttendanceRecord } from '../types';
...
export const createStudent = async (studentData: Partial<StudentRecord>) => {
  try {
...
export const getStudents = async () => {
  try {
    ...
    const students = studentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as StudentRecord));
...
export const getStudent = async (studentId: string) => {
  try {
    ...
    if (studentDoc.exists()) {
      return { success: true, data: { id: studentDoc.id, ...studentDoc.data() } as StudentRecord };
    } else {
...
export const updateStudent = async (studentId: string, studentData: Partial<StudentRecord>) => {
  try {
...
export const createSubject = async (subjectData: Partial<Subject>) => {
  try {
...
export const createMarks = async (marksData: MarkRecord) => {
  try {
...
export const getStudentMarks = async (studentId: string) => {
  try {
    ...
    const marks = marksSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as MarkRecord));
...
export const createAttendance = async (attendanceData: AttendanceRecord) => {
  try {
...
export const updateAttendance = async (studentId: string, subject: string, date: string, status: AttendanceRecord['status']) => {
  try {
    const attendanceId = `${studentId}_${subject}_${date}`;
...
export const getAttendanceByDate = async (subject: string, date: string) => {
  try {
    ...
    const attendance = attendanceSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as AttendanceRecord));
...
export const saveAttendanceBatch = async (attendanceRecords: AttendanceRecord[]) => {
  try {
